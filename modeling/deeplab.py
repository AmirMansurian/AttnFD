...
class DeepLab(nn.Module):
    ...
    def extract_feature(self, input):
        feats, attens, x, low_level_feat = self.backbone.extract_feature(input)
        feat, atten, x = self.aspp.extract_feature(x)
        feats += feat
        attens += atten
        feat, atten, x = self.decoder.extract_feature(x, low_level_feat)
        feats += feat
        attens += atten
        x = F.interpolate(x, size=input.size()[2:], mode='bilinear', align_corners=True)

        return feats, attens, x
